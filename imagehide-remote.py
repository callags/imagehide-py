...
DELIMITTER = '2222222222222220'
SUB_DELIMITTER = '2220222022202220'

def pixel():

    print("1. Red\n2. Green\n3. Blue\n")
    pixel_choice = sys.argv[3]
...
def decode(hexcode, pixel_choice):
    ...
    if pixel_choice == blue:
        ...
        if hexcode[-3] in ('0', '1', '2'):
                return hexcode[-3]
        else:
                return None

    elif pixel_choice == red:
        if hexcode[-5] in ('0', '1', '2'):
            return hexcode[-5]
        else:
            return None
            ...
            if digit == None:
                ...
                if retrieve == text:
                    ...
                    if (binary[-16:] == SUB_DELIMITTER):
                        end = (len(SUB_DELIMITTER))*-1
                        start = (binary.index(DELIMITTER,1)) + len(DELIMITTER)
                        filename_binary = binary[start:end]
                        filename = bin2str(filename_binary)
                        file_index = start - (len(DELIMITTER))
                        file_content = bin2str(binary[:file_index])
                        file_array = []
